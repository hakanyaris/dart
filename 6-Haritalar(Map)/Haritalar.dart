void main() {
  ...
  for (int i = 0; i < users.length; i++) {
    print(' $i ${users.keys.elementAt(i)}  ${users.values.elementAt(i)}');
  }
  //bazen maplarda anahtar değeri girmeden 0,1,2,3 indekstedi   v.b değerleri (anahtar ve değeri) almak için
  //entries yöntemini kullanırız.
  int birinciElamanDeger = users.entries.toList()[0].value;
  print('users mapı 0 ıncı elemanın valuesi(değeri): $birinciElamanDeger');
  //users mapı 0 ıncı elemanın valuesi(değeri): 20          çıktı
  ...
  for (var map in notlarim.entries) {
    print(
      "elemanlarımızın key değeri : ${map.key} ve value değeri : ${map.value}",
    );
  }
}
